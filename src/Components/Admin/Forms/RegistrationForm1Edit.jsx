...
import { UpdateStudent,uploadFile } from "../../ApiService/api";
...
export default function RegistrationForm1Edit() {
  const location = useLocation();
  const [dataFetched, setDataFetched] = useState(location.state);
  ...
  const formik = useFormik({
    initialValues: {
      stdID: dataFetched?.stdID,
      fname: dataFetched?.fname,
      lname: dataFetched?.lname,
      email: dataFetched?.email,
      telephone: dataFetched?.telephone,
      nic: dataFetched?.nic,
      dateOfBirth: dataFetched?.dateOfBirth,
      registered: dataFetched?.registered,
      isMale: dataFetched?.isMale,
      registrationStatus: dataFetched?.registrationStatus,
      guardianName: dataFetched?.guardianName,
      guardianTelephone: dataFetched?.guardianTelephone,
      addressNo: dataFetched?.addressNo,
      adl1: dataFetched?.adl1,
      adl2: dataFetched?.adl2,
      city:  dataFetched?.city,
      profilePhotoURL:dataFetched?.profilePhotoURL,
    },
  ...
  const save = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await UpdateStudent(formik.values); // call api.saveStudent
          ...
          if (response.data.code === "00") {
            ...
            Swal.fire({
              icon: "error",
              title: "NIC, telephone, and email are duplicated separately.",
            });
