...
function VehicleCard({vehicleData,interrupt,setInterrupt}) {
    ...
    const selecteDate = saveLicenceFormik.getFieldProps("issuedDate").value;

// Check if the selected date is an empty string
    const next = selecteDate === "" ? new Date() : new Date(selecteDate);

    next.setFullYear(next.getFullYear() + 1);
    next.setDate(next.getDate() - 1);

    const minForExpiry = next.toISOString().split('T')[0];
                                            ...
                                            <div className="p-4">
                                                ...
                                                <Row className="mt-3">
                                                    ...
                                                    >
                                                        ...
                                                        <Dropdown>
                                                            <Dropdown.Toggle variant="outline-secondary" size="sm" className="h-9 w-52">
                                                            ...
                                                            <Dropdown.Menu>
                                                                ...
                                                                <Dropdown.Item className="bg-blue-700 text-white" style={{ backgroundColor: 'gray' }}
                                                                               onClick={()=>{setShowModalAddInsuranceType(true);}}>
                                                                    New Insurance Type
                                                                </Dropdown.Item>
                                                            </Dropdown.Menu>
                                                        </Dropdown>
                                                ...
                                                <Modal show={showModalAddInsuranceType} onHide={()=>{setShowModalAddInsuranceType(false)}}>
                                                    <Form onSubmit={formikAddNewInsuranceType.handleSubmit}>
                                                        ...
                                                        <Modal.Footer>
                                                            <Button variant="secondary" onClick={()=>{setShowModalAddInsuranceType(false)}}>
                                                                Close
                                                            </Button>
                                                            <Button variant="primary" type="submit">
                                                                Save Changes
                                ...
                                <Modal.Body className="flex flex-wrap h-96 items-center justify-center overflow-y-scroll flex-row">
                                    {
                                        vehicleData?.insurances?.map((data, i) => (
                                            <div className ={(data?.validMonths === 0 ?"bg-red-100 p-4 w-96 items-center text-sm mb-4 rounded overflow-auto":"bg-green-100 p-4 w-96 items-center text-sm mb-4 rounded overflow-auto")} key={i}>
                                                {data?.validMonths === 0 && vehicleData?.data?.length ===1 ? (
                                                    <Row>
                                                        <div className="mb-1 text-danger italic flex-row flex gap-x-4 items-center" >
                                                            <AiFillWarning/>
