...
import {deleteNotification, getAllnotificationByID} from "../../ApiService/api";
...
const TrainerNavBar = () => {
    ...
    useEffect(() => {
        const fetchNotifications = async () => {
            const response = await getAllnotificationByID(sessionStorage.getItem('username'), 'TRAINER');
            if (response?.data?.code === '00') {
                const newNotifications = response.data.content;
                setNotifications(newNotifications);
                console.log("Notification +++++++++++++++++",newNotifications);
            }
        };
        fetchNotifications();
    }, []);
